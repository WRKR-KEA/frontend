...
export default function UserCreateTicketPage() {
  const [selectedService, setSelectedService] =
    useState('1차 카테고리를 선택해주세요.');
  const [selectedRequestType, setSelectedRequestType] =
    useState('2차 카테고리를 선택해주세요.');
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        setFirstCategories(
          response.result.categories.map((category: any) => category.name),
        );
      } catch (error) {
  ...
  const handleCreate = async () => {
    try {
      ...
      if (error) {
        ...
        showModal(
          error?.response?.data?.result?.title ||
            error?.response?.data?.result?.content,
        );
        return;
      }
